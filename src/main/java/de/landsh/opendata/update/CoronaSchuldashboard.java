...
Anzahl durchgeführter Testungen in der Schule	https://api.public.polyteia.de/data/e9e1a89fb2/?week=2021-W51
    ...
    public CoronaSchuldashboard(String id, DatasetUpdate update) {
        targetFile = null;//new File(update.getGeneratorArgs().get("targetFile"));
        code =null;// update.getGeneratorArgs().get("code");
    }

    public static void main(String[] args) throws IOException {
        new CoronaSchuldashboard("test", new DatasetUpdate()).workWeek("1f99d72f38", new File("/tmp/testungen-art-nachweis.csv"));
        new CoronaSchuldashboard("test", new DatasetUpdate()).workWeek("1b75fec02d", new File("/tmp/testungen-positiv.csv"));
        new CoronaSchuldashboard("test", new DatasetUpdate()).workWeek("e9e1a89fb2", new File("/tmp/testungen-in-schule.csv"));
    }
    ...
    private void workWeek(String code, File targetFile) throws IOException {
    ...
    private void work(String code, File targetFile) throws IOException {
        CloseableHttpClient client = HttpClients.createDefault();
        PrintStream out = new PrintStream(targetFile);
    ...
    String findDataByName(JSONArray data, String name) {
    ...
    public boolean generateDistributions(File directory) throws Exception {
        ...
        final String date = DateTimeFormatter.ISO_DATE.format(now);

        final List<List<String>> table;
        if (CODE_SCHULE_GESCHLOSSEN.equals(code)) {
            final JSONObject json = downloadRawData("https://api.public.polyteia.de/data/" + code + "/?schultag=" + date);
            table = convertTable(date, json, "Gesamt");
        } else if (CODE_SCHULE_BEINTRAECHTIGT.equals(code)) {
            final JSONObject json = downloadRawData("https://api.public.polyteia.de/data/" + code + "/?schultag=" + date);
            table = convertTable(date, json, "Gesamt");
        } else if (CODE_FAELLE_SCHULART.equals(code)) {
            final JSONObject json = downloadRawData("https://api.public.polyteia.de/data/" + code + "/?schultag=" + date);
            table = convertTable(date, json, "Neue Fälle insgesamt");
